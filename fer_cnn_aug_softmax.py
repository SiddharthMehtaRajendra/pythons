...
class FERDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if index < self.data_len:
            data_X = np.array(self.df['pixels'].values[index], dtype='float32')
            im = Image.fromarray(np.uint8(cm.gist_earth(data_X)*255))
            data_X = transforms.Resize((config.width,config.height))(im)
            data_X = transforms.ToTensor()(data_X)
        elif index < 2 * self.data_len:
            index = index - self.data_len
            data_X = np.array(self.df['pixels'].values[index], dtype='float32')
            im = Image.fromarray(np.uint8(cm.gist_earth(data_X)*255))
            data_X = transforms.RandomHorizontalFlip()(im)
            data_X = transforms.Resize((config.width,config.height))(data_X)
            data_X = transforms.ToTensor()(data_X)
        elif index < 3 * self.data_len:
            index = index - 2 * self.data_len
            data_X = np.array(self.df['pixels'].values[index], dtype='float32')
            im = Image.fromarray(np.uint8(cm.gist_earth(data_X)*255))
            data_X = transforms.RandomRotation(30)(im)
            data_X = transforms.Resize((config.width,config.height))(data_X)
            data_X = transforms.ToTensor()(data_X)
...
#             data_X = np.array(self.df['pixels'].values[index], dtype='float32')
#             im = Image.fromarray(np.uint8(cm.gist_earth(data_X)*255))
#             data_X = transforms.RandomAffine(5, translate=(0.1,0.1), scale=(1.1,1.2), shear=0, resample=False, fillcolor=0)(im)
#             data_X = transforms.Resize((config.width,config.height))(data_X)
#             data_X = transforms.ToTensor()(data_X)
#         else:
...
#             data_X = np.array(self.df['pixels'].values[index], dtype='float32')
#             im = Image.fromarray(np.uint8(cm.gist_earth(data_X)*255))
#             data_X = transforms.RandomAffine(5, translate=(0,0), scale=(1.1,1.2), shear=5, resample=False, fillcolor=0)(im)
#             data_X = transforms.Resize((config.width,config.height))(data_X)
#             data_X = transforms.ToTensor()(data_X)
            

        if data_X.shape[0] == 3:
            data_X = transforms.Grayscale(num_output_channels=1)(data_X)
            data_X = transforms.ToTensor()(data_X)
        
        data_X = transforms.Normalize(mean=[0.485], std=[0.229])(data_X)
        data_Y = int(self.df['emotion'].values[index])
        
        return data_X, data_Y
    
    
train_dataset = FERDataset(data_train, train=True) #training data
...
torch.save(model.state_dict(),'model_closs_20epochs.t7')
...
test_loss, test_accuracy = test_classify(model, test_dataloader)
